...
public static class Encryptor
{
    ...
    {
       return Convert.ToBase64String(EncryptorStringToBytes_Aes(text, key, iv));
    }
    ...
    private static byte[] EncryptorStringToBytes_Aes(string text, byte[] key, byte[] iv)
    {
        byte[] encrypted;
        ...
        using (Aes aesAlg = Aes.Create())
        {
            ...
            ICryptoTransform encryptor = aesAlg.CreateEncryptor(aesAlg.Key, aesAlg.IV);
            {
                ...
                {
                    using (CryptoStream csEncryptor = new CryptoStream(msEncrypt, encryptor, CryptoStreamMode.Write))
                    {
                        using (StreamWriter swEncrypt=new StreamWriter(csEncryptor))
                        {
